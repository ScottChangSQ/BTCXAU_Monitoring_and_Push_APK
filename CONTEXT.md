...
## 当前正在做什么
- 正在继续收口图表页交易交互细节：移除持仓行右侧展开提示、改成左侧点击展开明细、右侧只保留操作按钮，并修正“持仓操作”“买入/卖出/挂单”两类弹窗标题区和按钮区透出底层页面的问题。
- 已完成的最新结论是：`item_position.xml` 原先把“展开提示”和“操作按钮”都放在右侧，视觉冲突明显；同时为了修前一轮白底问题把交易表单弹窗窗口背景设成了透明，导致标题和“取消/继续”区域直接透出主界面。现已改为删除展开提示、保留右侧操作按钮，并统一把交易相关弹窗窗口背景收口成当前主题面板。
- 当前停点：`./gradlew.bat :app:testDebugUnitTest --tests "com.binance.monitor.ui.chart.MarketChartTradeSourceTest" --tests "com.binance.monitor.ui.chart.MarketChartRefreshSourceTest" --tests "com.binance.monitor.data.remote.v2.GatewayV2TradeClientTest" --tests "com.binance.monitor.ui.trade.TradeCommandFactoryTest" --tests "com.binance.monitor.ui.trade.TradeExecutionCoordinatorTest" --tests "com.binance.monitor.ui.trade.TradeCommandStateMachineTest" --tests "com.binance.monitor.ui.chart.MarketChartTradeSupportTest"` 与 `./gradlew.bat :app:assembleDebug -x lint` 已通过；新 APK 为 `app/build/outputs/apk/debug/app-debug.apk`，时间 `2026-04-06 18:10:32`。
- 本轮关键决定：不再保留任何“展开/收起”右侧文案，而是让左侧摘要区自然承担展开入口；原因是同一行右侧同时放“展开”和“平仓/改单”会继续造成视觉干扰和误触。
- 正在收口阶段一交易 UI 的现场问题：已开单没有可见平仓入口、交易弹窗白底且输入框显示不全、交易请求命中 `/v2/trade/check` 返回 404、图表页当前持仓区卡顿。
- 已完成的最新结论是：`404` 不是 Investor 账户导致，而是 Windows 部署包 `deploy/tencent/windows_server_bundle/mt5_gateway` 仍是旧版，缺少 `v2_trade.py`、`v2_trade_models.py`，并且包内 `server_v2.py` 没有 `/v2/trade/check`；APP 侧现已把 404 提示改成“网关未部署交易接口，请升级 server_v2.py 并重启 8787”。同时，图表页持仓操作入口之前只藏在长按手势里，现已改成行内可见操作按钮；交易弹窗已补独立深色容器和更高的输入控件；图表页账户叠加层新增短延迟去抖和签名跳过，减少重复重建造成的卡顿。
- 当前停点：`./gradlew.bat :app:testDebugUnitTest --tests "com.binance.monitor.data.remote.v2.GatewayV2TradeClientTest" --tests "com.binance.monitor.ui.chart.MarketChartTradeSourceTest" --tests "com.binance.monitor.ui.chart.MarketChartRefreshSourceTest" --tests "com.binance.monitor.ui.trade.TradeCommandFactoryTest" --tests "com.binance.monitor.ui.trade.TradeCommandStateMachineTest" --tests "com.binance.monitor.ui.trade.TradeExecutionCoordinatorTest" --tests "com.binance.monitor.ui.chart.MarketChartTradeSupportTest"`、`python -m unittest bridge.mt5_gateway.tests.test_gateway_bundle_parity bridge.mt5_gateway.tests.test_v2_trade_contracts`、`./gradlew.bat :app:assembleDebug -x lint`、`python -m py_compile bridge/mt5_gateway/server_v2.py bridge/mt5_gateway/v2_trade.py bridge/mt5_gateway/v2_trade_models.py deploy/tencent/windows_server_bundle/mt5_gateway/server_v2.py deploy/tencent/windows_server_bundle/mt5_gateway/v2_trade.py deploy/tencent/windows_server_bundle/mt5_gateway/v2_trade_models.py` 已通过；新 APK 为 `app/build/outputs/apk/debug/app-debug.apk`，时间 `2026-04-06 18:01:36`。
- 本轮关键决定：对 `404` 不做客户端兜底改旧路径，而是直接修正部署包并让客户端给出明确升级提示；原因是客户端和主仓库源码本来就已经走 `/v2/trade/check` 正确路径，真正缺的是服务器部署版本。
- 正在收口“管理后台看起来像白底纯文字”和“最新版 APK 没有交易 UI”这两个部署/产物问题，并把阶段一交易 UI 从 `codex-live-trading-phase1` 工作树同步回主目录。
- 已完成的最新结论是：`/admin/` 当前更像服务器仍在跑旧版静态资源或浏览器缓存了旧 `app.js/styles.css`，不是网关整体没起来；主目录 `E:\Github\BTCXAU_Monitoring_and_Push_APK` 之前缺少阶段一交易 UI 代码，导致编出来的 APK 还是旧版无交易入口。
- 当前停点：主目录已补齐 `GatewayV2TradeClient`、`TradeCommand*`、图表交易入口、交易弹窗布局，以及 `TradeCommand` / `PositionAdapterV2` / `PendingOrderAdapter` 的配套变更；`./gradlew.bat :app:testDebugUnitTest --tests "com.binance.monitor.ui.trade.TradeCommandFactoryTest" --tests "com.binance.monitor.ui.trade.TradeCommandStateMachineTest" --tests "com.binance.monitor.ui.trade.TradeExecutionCoordinatorTest" --tests "com.binance.monitor.ui.chart.MarketChartTradeSourceTest" --tests "com.binance.monitor.ui.chart.MarketChartTradeSupportTest"` 和 `./gradlew.bat :app:assembleDebug -x lint` 已通过，新 APK 为 `app/build/outputs/apk/debug/app-debug.apk`，时间 `2026-04-06 17:35:23`。
- 本轮关键决定：不再继续解释“为什么旧 APK 没交易按钮”，而是直接把工作树阶段一交易代码同步回主目录并重编 APK；原因是根因已经确认是主目录和工作树分叉，不是单纯打包失误。
...
- 本轮关键决定：这次先写“总实施计划”，不直接把四阶段展开成一份超大执行清单；原因是当前范围跨服务端、APP、图表、风控和审计，必须先按阶段切开，后面每阶段再单独出实施计划才不会失控。
