...
public class TradeRecordAdapterV2 extends RecyclerView.Adapter<TradeRecordAdapterV2.Holder> {
    ...
    public void submitList(List<TradeRecordItem> data) {
        ...
        if (data != null) {
            for (TradeRecordItem item : data) if (expandedKeys.contains(keyOf(item))) next.add(keyOf(item));
        }
        expandedKeys.clear();
    ...
    private String keyOf(TradeRecordItem item) { return item.getTimestamp() + "|" + item.getCode() + "|" + item.getSide(); }

    static class Holder extends RecyclerView.ViewHolder {
        private final ItemTradeRecordBinding binding;
        Holder(ItemTradeRecordBinding binding) { super(binding.getRoot()); this.binding = binding; }
        ...
        void bind(TradeRecordItem item, boolean expanded) {
            ...
            if (start >= 0) span.setSpan(new ForegroundColorSpan(sideColor), start, raw.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            binding.tvDetail.setText(String.format(Locale.getDefault(), "价格 $%s | 数量 %.2f | 成交额 $%s | 手续费 $%s",
                    FormatUtils.formatPrice(item.getPrice()), item.getQuantity(),
                    FormatUtils.formatPrice(item.getAmount()), FormatUtils.formatPrice(item.getFee())));
