...
public class AccountOverviewMetricsCalculatorTest {
    ...
    @Test
    public void calculateShouldFallbackToFreeMarginWhenPrepaymentMetricMissing() {
        List<AccountMetric> overview = Arrays.asList(
                new AccountMetric("可用保证金", "$650.00"),
                new AccountMetric("杠杆", "100x")
        );
        ...
        assertEquals(150d, values.getPrepayment(), 0.0001d);
        assertEquals(650d, values.getFreePrepayment(), 0.0001d);
    }

    @Test
    public void calculateShouldEstimatePrepaymentFromOpenPositionsWhenOverviewMarginMissing() {
        List<AccountMetric> overview = Arrays.asList(
                new AccountMetric("杠杆", "100x")
        );
        List<PositionItem> positions = Arrays.asList(
        ...
        assertEquals(398d, values.getPrepayment(), 0.0001d);
        assertEquals(402d, values.getFreePrepayment(), 0.0001d);
    }
}
