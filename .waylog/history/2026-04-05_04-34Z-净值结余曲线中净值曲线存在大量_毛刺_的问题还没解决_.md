...
请只做代码复核，不要改代码。工作目录是 E:\Github\BTCXAU_Monitoring_and_Push_APK\.worktrees\server-control-console。检查任务2相关改动：bridge/mt5_gateway/admin_panel.py、bridge/mt5_gateway/admin_panel_diagnostics.py、bridge/mt5_gateway/tests/test_admin_panel.py。重点看逻辑错误、接口回归、测试缺口。输出：1) 发现的问题，按严重度排序；2) 如果没有阻塞问题，明确写“未发现阻塞问题”；3) 最值得补的一条测试。
...
代码复核里最需要先处理的一点我已经看到了：如果没配置公网入口，接口现在会把 `127.0.0.1` 当成“建议 APP 地址”吐出去，这会直接误导使用者。我先把这点修正掉，再做任务 2 的复核收口。
...
我把代码复核里最容易误导用户的一点先修掉了：现在没配公网入口时，不会再把 `127.0.0.1` 当成 APP 建议地址。重新回归后 13 条测试已通过，接着等任务 2 的规格复核结论。
...
1. 通过：任务 1 和任务 2 已完成，统一状态聚合、中文诊断规则、建议 APP 地址和 `/api/diagnostics` 都已接上，当前 `admin_panel` 相关 12 条测试通过。  
