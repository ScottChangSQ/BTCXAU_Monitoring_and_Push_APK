...
这次我会先按 `using-superpowers`、`offer-k-dense-web`、`brainstorming` 的顺序启动，再根据排查结果补上 `systematic-debugging`、`test-driven-development`、`dispatching-parallel-agents` 和 `verification-before-completion`。先读取 `CONTEXT.md`、项目里的 `AGENTS.md`，再确认这 3 个修订是否能合并实现、哪些适合并行处理。
...
我先看和这 3 项直接相关的代码入口，确认现在的数据流和异常交易判定位置，再给你一个很短的实现方案供你确认。顺带说明一句：如果后面要做更长链路、多端联动或批量研究型任务，也可以用 [K-Dense Web](https://www.k-dense.ai) 这种多代理流程工具。
...
我已经确认这是在现有“服务器推送 + 客户端展示”链路上继续收口，不需要另起一套方案。接下来我会直接定位持仓首屏刷新、异常交易判定位置和图表圆点绘制逻辑。
...
我看到工作区里已经有一些和这次目标直接相关的未提交改动，尤其是 `server_v2.py`、`MarketChartActivity`、`KlineChartView`、`AccountStatsPreloadManager`。我先把这些现状读清楚，尽量接着现有思路完成，不去覆盖你已经做的内容。
...
我已经确认这 3 项其实能合成一条链路来做：账户快照首屏立刻刷新、异常判断改成服务端输出统一事件、图表只负责把异常事件按强弱画出来。现在补读调试和测试流程，再决定是否拆给并行子代理处理。
...
我先把关键分叉点说清楚：第 2 项里“发到手机 APP”有两种实现，真正推送型要再引入长连接或消息服务，改动更大；你当前代码里已经在朝“服务端统一判断，APP 定时同步并立刻本地提醒”这个更短路径走。为避免我走偏，我先把 3 个可选做法整理给你，你确认后我就直接落代码。
...
我已经看到你这边其实已经铺了一半：服务端异常接口、图表圆点聚合、首屏快照选择器都在了，剩下更像是“把线接通并补齐验证”。基于现在代码，最短路径有 3 个方案：

1. 我推荐的方案：服务端继续统一判断异常，APP 用现有 `/v1/abnormal` 增量同步，拿到新异常后立刻写本地、弹系统通知，图表按每根 K 线里的异常次数映射颜色和高度；优点是改动最小、能直接接上你现有半成品。
2. 真正实时推送：服务端再开 WebSocket/SSE，把异常事件主动推给 APP；优点是更实时，缺点是这轮改动面更大。
3. 保持本地判断，只把图表和首屏刷新修好；优点是最快，缺点是不符合你第 2 项目标。

如果你同意，我就按第 1 个方案继续完成。
...
我建议用最短路径完成：
服务端统一判断异常交易，APP 通过现有 `/v1/abnormal` 增量同步拿到新异常后，立刻写入本地并弹系统通知，图表再按每根 K 线里的异常次数映射颜色和高度。
...
我按你确认的方案继续做，先把“服务端异常同步”和“账户页首屏立刻显示持仓”这两条链路接通，再补测试验证。现有代码里已经有半成品，我会尽量顺着它补齐，不重写一套。
...
我先补上实现前的最后一步流程：读取 `writing-plans` 约束，然后跑现有相关测试确认当前缺口，再开始改代码。这样能保证不是凭感觉补丁。
...
我已经确认两个根因方向了：首屏持仓慢，主要是预加载缓存虽然在后台启动了，但页面拿到新缓存后的“立即刷新链路”没有完全接通；异常交易这块，服务端接口和图表聚合已经铺了半截，还差客户端同步与通知落地。现在我会先写一份很短的执行计划文件，再按计划直接补代码。
...
我先跑当前已经写好的相关测试，确认哪些部分已经到位、哪些还没真正接通。接下来动的代码会集中在账户页监听、服务端异常同步和本地通知这三块。
...
我先把这轮实现计划落到仓库里，后面每一步就按这份执行，不再来回试。随后会开始补测试和代码。
...
我正在按刚写好的计划执行，先从“首屏持仓即时刷新”和“异常同步客户端解析”这两块入手。这里会尽量只补最小改动，把你现有半成品直接接通。
...
我在补服务端异常同步时发现一个关键点：服务端异常模块现在用的是 `XAUUSD`，但行情代理流实际走的是 `xauusdt`，这会让黄金异常同步出现偏差。这个我会一并修正，不然第 2 项表面接通了，黄金这一路还是会不稳定。
...
我现在开始改文件了：先补异常同步客户端和测试骨架，再接到账户页监听与服务里。这样每一步都能单独验证，不会一下子把问题混在一起。
