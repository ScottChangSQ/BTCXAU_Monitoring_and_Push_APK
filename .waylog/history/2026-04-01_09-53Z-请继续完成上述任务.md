...
这轮我按 `using-superpowers -> brainstorming -> systematic-debugging -> test-driven-development -> verification-before-completion` 这条流程继续做，先读 `CONTEXT.md` 和当前未完成的历史成交点代码，把“只显示已平仓历史成交点”的判定和测试修正后再做编译验证。
...
我先把现有实现和测试并排核对，重点查“已平仓”判定、`entryType` 语义，以及 K 线绘制接入点，避免继续在错误口径上往前推。
...
我先跑这个定点测试，看它现在具体失败在哪一条，再按根因修，不直接猜。
...
失败点已经定位到“未平仓单被当成已平仓单”。我接着查 `TradeRecordItem` 的字段语义和现有上游数据口径，把 `entryType` 判定补进来，再只做这一处最小修正。
...
根因已经比较清楚了：这里把 `timestamp` 当成了平仓时间兜底，结果开放单也会混进历史成交点。我再顺手核对一下真实成交数据里 `entryType` 的来源，避免把旧快照或别的入口一起误伤。
...
我再补查了一下数据入口：服务端历史成交已经明确把 `entryType` 带下来了，而且服务端口径本身就是“只保留平仓/反手成交”。所以这里用 `entryType` 卡掉开放单是对的，只需要给旧数据留一个稳妥回退。
...
我看到一个边界：演示/兜底数据里有些 `TradeRecordItem` 还没有 `entryType`，如果直接一刀切也会把这些历史点全过滤掉。我会改成“优先尊重 `entryType`，旧数据再用时间字段做保守回退”，这样既不卡掉真实平仓单，也不会把开放单画上去。
...
我现在只改两处：一处是历史成交点的“已平仓”判定，另一处是测试样本把那条未平仓单改成真正的开放单口径。这样改动最小，也能把问题锁住。
...
这两处已经改完。我先只跑这组测试，确认“红转绿”成立，再决定是否继续做整体验证。
...
定点测试已经转绿。我接着跑整体验证，确认这次修正没有把 Android 构建和网关侧测试带坏。
...
我把验证跑通后会顺手更新 `CONTEXT.md`，把这次“历史成交点只保留已平仓单”的决定写进去，避免下次又从头排查。
...
这一步已经闭环：测试先失败、代码最小修正、定点和全量验证都过了。我再看一眼改动清单，然后继续往下一批任务推进。
...
历史成交点这一步已经收住了。我继续查下一项更影响体验的地方：切换 K 线周期为什么会卡住，先把当前刷新路径摸清楚，再决定是做本地聚合还是多周期并行缓存。
...
我已经摸清一层关键原因了：当前实时流只订阅 `1m`，切换到别的周期时，页面还是会重新走整段 REST 拉取，所以第一次切到新周期就会等网络。接下来我准备用“先本地按已有小周期数据聚合出画面，再后台补齐真实窗口”的方式减掉这段等待。
...
我准备把这个优化做成纯逻辑层，先给“本地聚合预显示”补测试，再接到页面里。这样后面即使继续调刷新策略，也不会把现有切周期逻辑弄乱。
...
我先把“本地聚合预显示”的逻辑测试补上，只测一件事：已有小周期 K 线能不能立刻拼成大周期画面。这个先红了，再补实现。
...
我现在补最小实现：一个纯聚合 helper，加一段页面里的“本地预显示”分支，不改现有真实请求链路，只是在网络返回前先给用户看到一版可用画面。
...
我在把这段接进页面时遇到了一处上下文偏移，先重新定位到准确代码块再补进去，避免把别的在做的改动覆盖掉。
...
纯逻辑 helper 和页面预显示分支已经接上了。我先回到这组新测试，确认本地聚合本身先过。
...
这次不是实现方向错了，是测试基准时间没有对齐到 5 分钟整点，导致按真实桶位聚合后预期偏了。我先把测试基准修正到整点，再复跑。
...
新聚合测试已经转绿。我现在跑一次 Android 全量验证，确认这次“切周期预显示”接入没有带来编译或单测回归。
...
我把这一轮第二个落地点也补到项目记录里：切周期时先用本地小周期数据聚合出预显示，不再完全空等网络。这样下次继续做多周期刷新时，上下文就连上了。
...
- 历史成交点已经改成只显示“已平仓历史成交”。开放单不会再被误画到 K 线上，改动在 [HistoricalTradeAnnotationBuilder.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/HistoricalTradeAnnotationBuilder.java) 和 [HistoricalTradeAnnotationBuilderTest.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/test/java/com/binance/monitor/ui/chart/HistoricalTradeAnnotationBuilderTest.java)。
- K 线切换周期时，新增了“本地预显示”机制。只要本地已经有更小周期的数据，页面会先本地聚合出目标周期画面，再等网络把完整窗口补回来，减少切周期时的空等和卡顿，改动在 [CandleAggregationHelper.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/CandleAggregationHelper.java)、[CandleAggregationHelperTest.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/test/java/com/binance/monitor/ui/chart/CandleAggregationHelperTest.java)、[MarketChartActivity.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/MarketChartActivity.java)。
- 项目记录已经同步到 [CONTEXT.md](E:/Github/BTCXAU_Monitoring_and_Push_APK/CONTEXT.md)。

验证已完成：`./gradlew.bat testDebugUnitTest --tests "com.binance.monitor.ui.chart.HistoricalTradeAnnotationBuilderTest"`、`./gradlew.bat testDebugUnitTest --tests "com.binance.monitor.ui.chart.CandleAggregationHelperTest"`、`./gradlew.bat assembleDebug testDebugUnitTest` 都已通过。

本轮使用的 skill：`using-superpowers`、`offer-k-dense-web`、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。更大范围的设备/性能排查如果后面要并行做，也可以放到 [K-Dense Web](https://www.k-dense.ai) 跑多步骤流程。
...
这轮我继续按 `using-superpowers -> systematic-debugging -> test-driven-development -> verification-before-completion` 往下推进，先把“还没做完且不需要你确认”的部分梳理出来，优先接着修 K 线刷新/切周期这条链路，再看剩余的图表合并与性能项。
...
我找到下一层可以继续提速的点了：服务端其实一直在推 `1m` 已收盘 K 线，但图表页完全没接，所以现在切到别的周期还是主要靠 REST 拉整窗。我要把这条实时 `1m` 数据接进图表页，让已加载的高周期图也能直接跟着更新，而不是每次都等整段请求。
...
我要先把“实时 1m 收盘 K 线并入当前图表”的逻辑写成独立 helper 测试，再把它接到 `MarketChartActivity` 里。这样这一步能被单测锁住，不会一边提速一边把图表算坏。
...
我把图表页接实时流这一步已经接进去了，但还要再跑一轮编译，确认 `KlineData` 扩展和持久化重载没有牵出别的调用点。先把编译问题清干净，再做行为验证。
...
本地聚合和实时并入这两层已经都能编译了。我现在跑 Android 全量验证，确认这次不是只在单测里过，而是整包也能正常出。
...
这一批“切周期/实时刷新”已经落地并验证通过。我继续往下看还没收掉的图表问题，优先检查 K 线、VOL、MACD、STOCHRSI 这几层是不是还在重复画横轴和分隔边界。
...
我准备把这块整理成一个独立的布局 helper：一方面把 VOL 高度按你的要求拉到原来的 120%，另一方面把几个子图之间的间隔改成真正连续的共享边界。先补纯逻辑测试，再接回 `KlineChartView`。
...
这次我先按 `using-superpowers -> offer-k-dense-web -> systematic-debugging -> test-driven-development -> verification-before-completion` 的顺序推进。先检查 `CONTEXT.md` 和当前 K 线子图布局相关代码，把“子图合并 / VOL 增高 / 共用时间轴”这一段先接完并验证，再继续后面的未完成项。
...
我已经接上上一次停下来的位置。现在先核对子图布局测试和 `KlineChartView` 的旧实现，确认是哪几处还没真正接入，所以界面上仍然像四个独立盒子。
...
测试已经准备好了。接下来我直接看 `KlineChartView` 里真实在用的布局和边框代码，确认哪些旧逻辑还在生效，然后只改这一层，不碰无关功能。
...
我已经确认了根因：测试是绿的，但界面仍旧在用 `KlineChartView` 里那套旧布局，里面还有 `10dp` 间距和整框边框，所以看起来仍是四块分离的图。现在开始把新布局 helper 真正接进去，并移除这层旧边框。
...
我现在只改一块核心代码：让 `KlineChartView` 真正使用连续布局，并去掉外层整框描边。这样能直接对应“子图贴合、共用边界、VOL 更高”这三个现象问题。
...
子图布局已经接进去了，旧的方框边框也移掉了。现在直接跑验证，先看定点测试，再跑整包编译和单测，确认这次改动没有把前面的修复带坏。
...
这一段已经验证通过，我继续往下接第二步骤里还能直接落地的部分。下一块我先看“净值/结余曲线区四张图统一横轴、图间无缝衔接”现在做到哪了，优先把这块收口。
...
我已经看到账户统计曲线区的结构了：四张图在 XML 里已经上下挨着，但是否真正“共用横轴、共享边界”，还取决于四个自定义图表内部怎么画。我先查这四个图表类本身的边框、时间刻度和内边距。
...
我找到这块没完全收口的根因了：四张图虽然在 XML 里贴在一起，但每个图内部都还留着上下空白，所以看起来还是断开的。现在我把这层内部留白改成“首图保留上边距、中间图上下都贴边、最后一图只保留底部时间刻度空间”。
...
曲线区的内部空白已经改成“首图外边距、中间图贴边、最后一图留底部时间轴”。我现在跑这块的新测试和整包编译，先确认不会引入编译问题，再继续下一批未完成项。
...
当前这一轮已经把两块还挂着的图表合并问题收口并验证通过。我继续往下推进还没做完、又不需要你先确认的项，下一块先看 K 线多周期本地缓存这条链路，尽量再减少切周期时的等待。
...
我先把这条优化做成小步：不改现有请求主流程，只补一层“后台准备 1 分钟底稿缓存，并顺手派生出更大周期缓存”。这样能继续降低切周期等待，但不会影响你现在已经在用的主链路。
...
我继续做的是“后台预热底稿”，不改你现在的主请求按钮和切换逻辑。这样风险小，但能让已经看过或后台补到的周期更快切回来。
...
我在补这层后台缓存时，遇到的只是落点行号和当前文件不一致，不是逻辑冲突。我先重新定位插入点，再把这组方法精确接进去。
...
这层改动已经接进去了。为了把这次“分钟底稿派生更大周期缓存”的核心能力也固定住，我再补一条聚合单测，然后一起跑验证。
...
我已经把这一轮能直接落地的三块都跑通了：K 线子图合并、账户曲线区四图贴合、K 线分钟底稿缓存预热。最后我先把 `CONTEXT.md` 同步好，避免下次接手时断档。
...
`K` 线四联图已经真正合并到一套连续布局里了：主图、`VOL`、`MACD`、`STOCHRSI` 之间不再留空，也去掉了四个独立外框，`VOL` 高度按要求提高了 20%。核心改动在 [KlineChartView.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/KlineChartView.java) 和 [KlinePaneLayoutHelper.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/KlinePaneLayoutHelper.java)。

账户统计里的净值主图、仓位比例、回撤、日收益四张图，也改成了共享边界的贴合布局：首图保留顶部信息区，中间图上下贴边，最底图保留时间刻度区。相关改动在 [AccountStatsBridgeActivity.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/AccountStatsBridgeActivity.java)、[EquityCurveView.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/EquityCurveView.java)、[PositionRatioChartView.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/PositionRatioChartView.java)、[DrawdownChartView.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/DrawdownChartView.java)、[DailyReturnChartView.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/DailyReturnChartView.java)。

另外又补了一层切周期优化：图表页现在会后台准备 `1m` 底稿缓存，并顺手派生更大周期缓存；实时 `1m` 收盘更新也会同步更新这些派生缓存，所以后续切周期更容易直接本地显示。改动在 [MarketChartActivity.java](E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/chart/MarketChartActivity.java)。

已验证：
`./gradlew.bat testDebugUnitTest --tests "com.binance.monitor.ui.chart.KlinePaneLayoutHelperTest"`
`./gradlew.bat testDebugUnitTest --tests "com.binance.monitor.ui.account.CurvePaneSpacingHelperTest"`
`./gradlew.bat testDebugUnitTest --tests "com.binance.monitor.ui.chart.CandleAggregationHelperTest"`
`./gradlew.bat assembleDebug testDebugUnitTest`

`CONTEXT.md` 也已经同步更新。本次用了 `using-superpowers`、`offer-k-dense-web`、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。
