...
我理解你的需求是：在“账户统计 → 交易记录 → 明细条目”里，把上下滚动条做成可手动拖拽（现在拖不到或还没实现）。我先快速定位这块页面和当前滚动条实现，再直接改成可拖拽并自测。
...
我已经定位到是 Android 工程，接下来会在页面代码里找“账户统计/交易记录/明细条目”和当前滚动条实现；`rg` 在当前环境不可用，我改用 PowerShell 递归检索继续排查。
...
我找到了和滚动条相关的资源文件（`bg_fast_scroll_*`），说明项目里已经有一套自定义滚动条。我现在正在定位“交易记录明细列表”到底接的是哪个 `RecyclerView`，然后把拖拽事件补上。
...
已经定位到这块：交易明细区使用了 `recyclerTrades + seekTradeScroll(旋转的 SeekBar)`。我现在查看 `AccountStatsActivity` 里它们的联动逻辑，判断是“没绑定拖拽回调”还是“触摸被拦截”。
...
我确认当前真正在运行的是 `AccountStatsBridgeActivity`（`AndroidManifest` 已指向它），所以我会在这个页面里改，不会改错到旧版 `AccountStatsActivity`。接下来我直接检查它对 `seekTradeScroll` 的事件绑定。
