# 你是 Task 4 的 implementer。你不孤单，代码库里可能还有别的改动；不要回退别人的编
...
你是 Task 4 的 implementer。你不孤单，代码库里可能还有别的改动；不要回退别人的编辑，只在你负责的文件内完成这次任务，并适配现有代码风格。

仓库：E:\Github\BTCXAU_Monitoring_and_Push_APK
分支：codex/remote-mt5-session
当前最新提交：fb37745（Task 3 已完成）

你的职责文件：
- app/src/main/java/com/binance/monitor/data/model/v2/session/SessionPublicKeyPayload.java
- app/src/main/java/com/binance/monitor/data/model/v2/session/RemoteAccountProfile.java
- app/src/main/java/com/binance/monitor/data/model/v2/session/SessionStatusPayload.java
- app/src/main/java/com/binance/monitor/data/model/v2/session/SessionReceipt.java
...
- 现有 app 侧 model / test 代码风格

必须遵循：
1. 先做根因分析，再改。
2. 严格 TDD：先新增失败测试，再实现最小代码，再跑测试。
3. 代码注释中文，保持 UTF-8。
4. 只做 Task 4，不碰 Task 5 的 Activity/状态机/UI。
...
2. 新增 GatewayV2SessionClient，至少支持：
- parseSessionPublicKey(String body)
- parseSessionStatus(String body)
- parseSessionReceipt(String body)
- fetchPublicKey()
- fetchStatus()
- login(LoginEnvelope envelope, boolean saveAccount)
- switchAccount(String profileId, String requestId)
- logout(String requestId)

3. 新增 SessionCredentialEncryptor，必须匹配服务端 Task 3 已实现的 `rsa-oaep+aes-gcm`：
- 读取 PEM 公钥
- 生成随机 AES key 与 iv
- AES-GCM 加密 JSON 载荷
- RSA-OAEP(SHA-256) 加密 AES key
- 生成 App 侧 LoginEnvelope 对象（如果你需要新建内部模型，可放在本任务职责目录里，但尽量最小）
- 载荷里至少包含：login/password/server/remember/nonce/clientTime

4. 单测至少覆盖：
- `parseSessionPublicKeyShouldKeepKeyAndAccounts`
- session receipt / status 解析稳定
- encryptor 输出 `keyId/algorithm/encryptedKey/encryptedPayload/iv/requestId`
- encryptor 生成的 envelope 能被结构化检查，且同一输入每次 `nonce/requestId/iv` 不必相同

测试命令：
- `gradlew testDebugUnitTest --tests "com.binance.monitor.data.remote.v2.GatewayV2SessionClientTest" --tests "com.binance.monitor.security.SessionCredentialEncryptorTest"`
...
我会按你的要求只做 Task 4，并严格走 TDD（先写失败测试再实现）。先做根因分析：我会先读取 `CONTEXT.md` 和项目规则文件，再检查你指定的 8 个职责文件现状，然后给出最小改动路径。  
这次我将按这个 skill 顺序执行：`using-superpowers`（会话启动规范）→ `brainstorming`（先澄清目标与边界）→ `test-driven-development`（先测后改）→ `verification-before-completion`（完成前核验）。
...
我已经读完 `CONTEXT.md`，当前上下文与 Task 4 边界清楚。下一步我会快速读取这次会用到的 skill 说明和你指定的职责文件现状，先完成根因分析，再进入“先写失败测试”的阶段。
...
我已经确认分支和提交点正确，且你说的几个目标文件目前多数还不存在，这符合 Task 4“新增”的预期。接下来我会先读取现有 `GatewayV2Client` 和相关测试风格，完成根因分析后再开始先写失败测试。
...
请给一个简短进度：是否已经完成 RED 测试、是否已经进入实现、是否卡住。
...
- RED 测试：**还没开始写**（我刚完成上下文和服务端契约核对，正在收敛根因分析）。
- 是否进入实现：**还没进入实现代码阶段**（会先写并跑失败测试再实现）。
- 是否卡住：**没卡住**，边界清晰，下一步就开始补 `GatewayV2SessionClientTest` 和 `SessionCredentialEncryptorTest` 的 RED 用例。
...
我已经完成根因分析，结论是：Task 4 当前缺口是“会话模型 + 会话客户端 + 加密器”整块缺失，不是单点 bug。接下来按最短闭环执行：先新增两组 RED 测试并跑到失败，再补最小实现，最后按你给的命令验证并更新 `CONTEXT.md`。
...
现在开始进入 RED 阶段：我会先只新增两个测试文件（会话客户端解析 + 加密信封结构），不写实现代码，然后立刻运行你指定的测试命令确认失败。
...
RED 测试已写好，我现在按你指定命令直接运行，目标是先看到失败（类未实现或方法缺失），确认测试确实能卡住需求。
...
RED 已确认：测试按预期失败（缺少会话模型、客户端、加密器类）。我现在进入 GREEN 阶段，开始补 6 个主代码文件的最小实现，让这两组测试先转绿。
...
未完成，暂无卡点（RED 已通过，正在进行 GREEN 实现）。
...
主工作区复核结果：Task 4 当前只有测试文件落地，实现文件还没进主工作区。现状 `git status` 只看到：
- ?? app/src/test/java/com/binance/monitor/data/remote/v2/GatewayV2SessionClientTest.java
- ?? app/src/test/java/com/binance/monitor/security/
...
说明你这轮还没有把 GREEN 实现真正落到主工作区。请立即继续完成 Task 4，并在完成后给最终 DONE / DONE_WITH_CONCERNS，包含：
- 根因分析
- 改了哪些文件
- 测试命令与结果
- 如果有额外依赖或边界限制，一并说明。
...
收到，我将直接继续 GREEN 实现并落到主工作区：先补齐 6 个实现文件，再跑你给的同一条测试命令验证，最后给你完整 DONE 报告。
...
我开始写实现文件了：会先补四个会话模型，再补会话客户端和加密器，保持现有代码风格和最小实现。
...
测试编译失败的根因已定位：`JSONObject.put` 的受检异常未处理。我会做最小修复（让明文载荷构建方法显式抛异常并由 `encrypt` 透传），然后重跑同一命令。
