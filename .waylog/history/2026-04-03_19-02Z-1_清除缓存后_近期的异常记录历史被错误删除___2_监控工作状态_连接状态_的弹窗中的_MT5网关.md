...
你在同一仓库里协作，只做只读分析，不修改文件。目标：在 MarketChartActivity / KlineChartView / MarketChartDisplayHelper 里找出“最适合下一步合并成单一路径”的一个具体点，优先是历史分页去重/合并、预显示恢复、自动刷新三者中的重复逻辑。请只给 1 个最值得做的点，并说明：1) 具体文件和方法；2) 为什么重复；3) 建议的最小改法。
...
我现在直接收“历史分页去重/合并”这一层，把 `MarketChartActivity` 和 `KlineChartView` 里重复的 older 过滤逻辑合并成一个入口，避免同一条规则分散在两处。
...
我先补一个小测试，把“历史分页只追加真正更早且未存在的 K 线”固定下来，然后把这条规则抽成公共 helper，给 `Activity` 和 `KlineChartView` 共用。
...
我这里碰到一个小岔路，不是架构问题，是仓库里已经有一份同名方向的旧测试残留，但还没对应实现。我先对齐这部分命名和实现，再继续收主线。
...
仓库里已经有一份同方向测试了，我直接沿用已有命名 `ChartHistoryPagingHelper`，避免后面留下两套 helper 名字。
...
我现在把 `Activity` 和 `KlineChartView` 里那段重复的 older 过滤都切到同一个 helper，上层和视图层不再各写一套。
...
这一步已经把“历史分页 older 过滤”正式合成单一路径了。接下来我把记录补上并重新编译 APK，然后继续往下收剩余分叉。
...
这轮我继续往下执行了，已经把图表历史分页里最后一块明显重复逻辑收成单一路径：
