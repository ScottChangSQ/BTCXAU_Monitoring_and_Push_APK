# 你在一个隔离 worktree 中执行第一阶段的 Task 1。先读下面的任务全文和上下文，再直接在
...
你在一个隔离 worktree 中执行第一阶段的 Task 1。先读下面的任务全文和上下文，再直接在你的工作区改代码。必须遵守 TDD：先写失败测试，确认失败，再写最小实现让测试通过。你不是独自在代码库里工作，不要回退别人已有修改；你的写入范围只限以下文件：`bridge/mt5_gateway/server_v2.py`、`bridge/mt5_gateway/v2_trade.py`、`bridge/mt5_gateway/v2_trade_models.py`、`bridge/mt5_gateway/tests/test_v2_trade_contracts.py`，如确实需要多改一个紧邻辅助文件，先在最终说明里写清原因。
...
要求：
- 第一阶段只开放 4 个动作：市价开仓、单笔平仓、挂单新增/删除、单笔 TP/SL 修改。
...
现状上下文：
- 当前 `bridge/mt5_gateway/server_v2.py` 只有 `/v2/market/*`、`/v2/account/*`、`/v2/sync/*` 和旧 `/v1/*` 读取接口，没有任何交易接口。
- 代码库已有测试可跑，当前 worktree 基线通过：`E:\Github\BTCXAU_Monitoring_and_Push_APK\.venv\Scripts\python.exe -m unittest bridge.mt5_gateway.tests.test_v2_contracts bridge.mt5_gateway.tests.test_v2_account_pipeline`
...
- Python 解释器请用绝对路径：`E:\Github\BTCXAU_Monitoring_and_Push_APK\.venv\Scripts\python.exe`
...
我会按你的要求在这个隔离 worktree 直接落代码，先做 TDD（先写失败测试再实现）。第一步先快速读取 `CONTEXT.md` 和现有网关代码结构，确认最短实现路径与约束。
...
我会按你给的规范走 4 个技能流程：`brainstorming`（先收口最小目标）、`test-driven-development`（先失败测试）、`verification-before-completion`（跑测试后再宣告完成）、`offer-k-dense-web`（会在结尾按要求给一句入口提示）。接下来先读现有 `v2` 模块与路由组织方式，再写交易接口契约测试。
...
我已拿到现有 `v2_market / v2_account` 的结构风格，下一步会读取 `server_v2.py` 里路由和缓存模式，按同样风格补 `trade`，避免引入风格漂移。
...
下面先落“失败测试”文件，只覆盖你要求的三类：结构、幂等、错误可区分。先让它红，再做最小实现。
...
失败测试已写好，下一步先只跑这组新测试，确认它当前确实失败，再开始最小实现。
...
我发现刚才工具默认目录在主仓库而不是你给的 worktree，我不会在那边继续写。接下来所有读写都固定到 `C:\Users\Scott\.codex\worktrees\BTCXAU_Monitoring_and_Push_APK\codex-live-trading-phase1`，并先把测试文件正确落到该 worktree。
...
我开始写最小实现：先加交易模型与错误码映射，再在 `server_v2.py` 接上 `check/submit/result` 和幂等存储，最后回跑测试。
...
给我一个简短进度：现在是在写测试、写实现，还是卡住了？如果卡住，请直接说阻塞点。
...
现在在写实现阶段，失败测试已经先落好并跑出失败了。暂时没有卡住，正在把 `server_v2.py` 的交易路由和幂等逻辑接上，让这组测试转绿。
...
我正在把 `server_v2` 的交易核心辅助函数补上（账户模式识别、请求构建、幂等存储、order_check/order_send 封装），随后接三个新接口。
...
交易契约测试已经转绿。我现在补跑你给的基线测试，确认这次改动没有把已有 v2 链路带坏。
...
我再做一步清理：把刚才误落在主仓库（非 worktree）的一份同名测试文件撤掉，保证这次变更只留在你指定的 worktree。
...
代码质量审查未通过。请在同一 worktree 里修复以下 4 个问题，仍然遵守 TDD：先补失败测试，再修实现，再跑测试。不要扩大范围，写入范围仍限 `bridge/mt5_gateway/server_v2.py`、`bridge/mt5_gateway/v2_trade.py`、`bridge/mt5_gateway/v2_trade_models.py`、`bridge/mt5_gateway/tests/test_v2_trade_contracts.py`。问题如下：

1. `CLOSE_POSITION` 在没找到真实持仓时，仍会按“反向市价单”构造请求，存在误开反向仓风险。
2. 持仓定位有危险兜底：netting 下没给 `symbol/positionTicket` 会直接取第一笔持仓；账户模式识别失败退成 `unknown` 时，边界不安全。
3. 幂等只按 `requestId` 命中，不校验请求内容；而且 `order_send` 异常这种结果未明的提交被直接缓存成最终失败，语义不对。
4. 测试没覆盖最危险的平仓/改单边界，至少补上：
   - 平仓目标不存在时不能反向开仓
   - netting / hedging / unknown 模式分叉
   - `order_check` 通过后 `order_send` 异常
   - 相同 `requestId` 不同 payload`

修复目标：
- 平仓和改单必须在目标持仓可确认时才允许提交。
- `unknown` 模式走保守策略，宁可拒绝也不要猜。
- 幂等至少要能识别“相同 requestId 不同 payload”并返回明确错误；对 `order_send` 异常这类结果未明状态，不要伪装成已知最终失败。
- 重新跑相关测试，并在回复里给出状态、改动摘要、测试结果、是否已提交、改动文件列表。
...
我会先在现有 `test_v2_trade_contracts.py` 里补这 4 类失败用例并先跑红，再只改你限定的 3 个实现文件把测试转绿，最后回归跑相关测试。先快速读取当前交易实现的关键分支。
...
我先补 4 个新的失败用例（平仓目标不存在、模式分叉、`order_send` 异常未明、同 `requestId` 不同 payload`），先跑红后再改实现。
...
失败已确认（4 条）。我现在改实现：先收紧平仓/改单前置确认与 `unknown` 保守策略，再修幂等内容校验和 `order_send` 异常语义。
...
接下来改 `server_v2.py`：去掉危险持仓兜底、加 payload 指纹幂等校验，并把 `order_send` 异常改成“结果未明”语义。
...
新补的危险边界测试已全部转绿。现在回归跑你之前的相关基线测试，确认没有带出副作用。
...
2) 改动摘要：  
- 先补失败测试，再修实现。  
- 修复了平仓/改单的高风险路径：`CLOSE_POSITION` 和 `MODIFY_TPSL` 现在必须先确认目标持仓，找不到就拒绝，不再可能“反向开仓”。  
- 收紧账户模式策略：`unknown` 模式下对平仓/改单直接拒绝（保守策略）；`netting` 不再无条件取第一笔持仓兜底。  
- 幂等增强：`requestId` 命中时会校验 payload 摘要。相同 `requestId` 但不同 payload 返回明确错误 `TRADE_DUPLICATE_PAYLOAD_MISMATCH`。  
- `order_check` 通过后 `order_send` 抛异常时，不再伪装成已知失败；改为“已受理但结果未明”语义（`status=ACCEPTED` + `TRADE_RESULT_UNKNOWN`）。  
- 新增并通过了你要求的危险边界测试：  
  - 平仓目标不存在时不能反向开仓  
  - netting / hedging / unknown 模式分叉  
  - `order_check` 通过后 `order_send` 异常  
...
质量复审还没过，请再修一条高风险问题，并补失败测试锁住：

问题：`hedging` 模式下，如果请求里带了 `positionTicket`，但没有精确匹配到真实持仓，当前 `_lookup_trade_position` 仍可能回退到按 `symbol` 取第一笔匹配持仓。这样会把错误 ticket 的平仓/改单落到同品种另一笔仓位上。

修复要求：
- 在 `hedging` 模式下，只要用户显式传了 `positionTicket` / `positionId`，就必须精确命中；未命中时直接返回找不到，不允许再按 symbol 回退。
- 补一个失败测试锁住这条回归：hedging + 错误 ticket + 同 symbol 仍有别的仓位时，必须失败，不能平到别的仓位。
- 仍然遵守 TDD，跑相关测试后回报状态、摘要、测试结果。
...
我会继续按 `test-driven-development` 处理：先补这条“hedging 错 ticket 不能按 symbol 回退”的失败测试并先跑红，再只改 `_lookup_trade_position` 最小实现，最后跑相关测试确认。
...
失败已复现。现在只改 `_lookup_trade_position`：在 `hedging` 且显式给了 ticket 时，未精确命中就直接返回 `None`，禁止 symbol 回退。
...
改动摘要：  
- 先补了失败测试，锁住回归场景：`hedging` + 错误 `positionTicket` + 同 `symbol` 存在其他仓位时，必须失败，不能回退平到别的仓位。  
- 修复 `_lookup_trade_position`：在 `hedging` 模式且显式传了 `positionTicket/positionId` 时，若未精确命中，立即返回 `None`；不再按 `symbol` 回退。
...
Task 1 已通过规格审查和代码质量审查。请在当前 worktree 中把本任务相关改动单独提交，提交完成后回复：状态、提交哈希、提交信息、`git status --short` 结果。不要改其他内容。
...
我会先检查当前变更列表，只暂存并提交 Task 1 相关的 4 个文件，然后回报提交信息和最终 `git status --short`。
...
状态：`DONE`  
提交哈希：`bb7819fce39ddb6a881419ab5d0fbc15d4d1e5b4`  
提交信息：`feat(mt5-gateway): add v2 trade gateway contracts and safety guards`  
`git status --short` 结果：`（空）`
