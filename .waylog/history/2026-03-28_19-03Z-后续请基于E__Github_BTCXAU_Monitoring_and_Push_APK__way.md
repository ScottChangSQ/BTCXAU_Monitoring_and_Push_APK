...
后续请基于E:\Github\BTCXAU_Monitoring_and_Push_APK\.waylog\history文件夹下的历史记录进行后续操作。将账户统计-交易记录-明细条目部分的上下进度条增加手动拖动功能（目前点不到拖动、或者干脆还没有这个功能）；
...
我会先基于你指定的 `E:\Github\BTCXAU_Monitoring_and_Push_APK\.waylog\history` 历史记录快速确认相关页面和之前改动背景，然后定位“账户统计-交易记录-明细条目”里上下进度条的实现位置，再直接补上可手动拖动并验证交互。接下来先做代码定位。
...
我已经定位到你这次需求对应的历史记录文件（包含 3 月 28 日晚上几次同主题修改），下一步我会先读这些记录，严格按里面已有约定延续改动，避免偏离你之前的实现路线。
...
我已经确认到上一轮在 `AccountStatsBridgeActivity` 继续改动的线索，接下来我会先看当前工作区实际代码状态（防止有中断残留），然后只在真实生效页面上补齐拖拽事件。
...
我看到代码里其实已经有 `seekTradeScroll` 的回调了，所以“拖不动”更像是触摸命中区域或父容器拦截问题，不是完全没逻辑。我先把对应 Java 和 XML 片段读出来，确认具体卡点后再做精准修改。
