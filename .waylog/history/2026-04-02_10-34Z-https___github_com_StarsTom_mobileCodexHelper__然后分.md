# https://github.com/StarsTom/mobileCodexHelper， 然后分
_Exported on 04/02/2026 at 19:00:25 GMT+8 from OpenAI Codex via WayLog_
...
https://github.com/StarsTom/mobileCodexHelper， 然后分析依赖并安装调试运行。
...
我会先按项目规则读取 `CONTEXT.md` 和目录信息，确认当前工作区情况，再决定是直接在现有目录内拉取这个仓库还是单独放置。会用到的 skill：`using-superpowers`、`offer-k-dense-web`；如果运行时出现问题，再补用 `systematic-debugging` 和 `verification-before-completion`。
...
当前工作区是另一个 Android 项目，我不会把外部仓库直接塞进这里，先单独拉到同级目录，避免把现有项目弄脏。接着我会看它的说明文件和依赖清单，判断正确的安装与启动方式。
...
仓库已经单独拉下来了。现在先读取它自己的规则和说明，再看依赖文件，避免按错技术栈去装环境。
...
说明里已经能看出这是“Python 桌面工具 + Node 服务 + nginx + Tailscale”的组合，但还要确认源码版是不是缺少上游网页服务源码。这个点很关键，因为它决定能不能直接跑起来。
...
现在关键结论已经出来了：这个仓库本身不包含真正要跑的网页服务源码，它只是“桌面控制工具 + 对上游项目的覆盖补丁”。所以要跑起来，还得把指定版本的上游项目补齐。
...
下一步我把指定的上游版本补齐，再检查本机有没有 `Node / npm / nginx / Tailscale / Python`，这样就能判断是“缺源码”还是“缺运行环境”。
...
现有环境里最明显的差异有两个：`Node` 已装，但版本比说明里的 22 更高；`nginx` 和 `Tailscale` 还没装。先看上游项目本身对 Node 的要求，再决定是直接装依赖还是先降版本。
...
我先按它自己的流程把补丁覆盖到上游源码，再装 Node 依赖。这样如果后面跑不起来，就能直接判断是环境问题还是项目本身的问题。
...
依赖已经装上了。下一步是前端构建和本地启动验证，如果页面能在 `127.0.0.1:3001` 打开，说明核心服务已经跑起来。
...
构建完成了，虽然有一组 CSS 警告，但没有拦住产物生成。现在启动服务并直接访问本地地址，验证它是不是已经能正常提供页面。
...
核心网页服务已经起来了，本地地址返回 `200`。我再补看两个点：一是桌面控制工具能不能正常启动，二是 `nginx` 缺失会影响到哪一步。
...
现在已经确认两件事：网页服务能访问，桌面工具进程也能起来。剩下没打通的是 `nginx` 这一层，它影响的是“整套代理链路”和更接近手机访问的那部分。
...
我把 `nginx` 也补上，这样至少能把它要求的本地代理链路跑通。`Tailscale` 先不动，因为那一步通常还要你本人登录账号，不适合我替你自动接管。
...
`nginx` 已经装上了，只是当前这个终端还没刷新到新路径。我把它的实际安装位置找出来，再直接用绝对路径做一次启动验证。
...
我直接在当前终端里带着绝对路径去跑一遍 `nginx` 启动脚本。这样不用等 PATH 刷新，也能确认代理层是不是能正常起来。
...
我再把当前实际监听的端口和进程记下来，避免最后只知道“能打开”，却不知道是谁在跑。
...
我会补一个极简 `CONTEXT.md`，把这次已经验证过的状态记下来。这样你下次接着看时，不需要重新猜现在到底跑到了哪一步。
...
我准备收口前，再按“完成前验证”的流程补做一次检查，确保最后给你的不是“看起来像成功”，而是已经实际验证过的结果。
...
已把仓库拉到 `E:\Github\mobileCodexHelper`，并按它自己的说明把上游 `claudecodeui v1.25.2` 补齐、覆盖补丁、安装依赖、完成构建和本地启动验证。现在这套东西已经能在本机跑起来：

- 主服务：`http://127.0.0.1:3001`，实测返回 `200`
- `nginx` 代理：`http://127.0.0.1:8080`，实测返回 `200`
- 桌面控制工具：`python mobile_codex_control.py` 已能启动
- 我另外补了一个极简进度文件：`E:\Github\mobileCodexHelper\CONTEXT.md`

这次实际确认到的依赖情况是：
- 必需：Python、Node/npm、上游 `claudecodeui` 源码
- 本机已装并跑通：Python 3.14、Node 24、npm、nginx 1.29.7
- 还没打通：`Tailscale`，所以“手机走私网远程访问”这一步还需要你本人后续安装并登录

有两点你需要知道：
- `npm install` 成功了，但上游依赖里有 `35` 个安全告警，这是上游包的提示，没有阻止本次运行
- 前端构建时有一组 CSS 警告，但构建成功，页面也已经能打开

如果你现在要继续用，直接在浏览器打开 `http://127.0.0.1:3001` 做首次注册即可。  
如果你要停掉服务，可用 `powershell -ExecutionPolicy Bypass -File scripts\stop-mobile-codex-stack.ps1`。

本次用了这些 skill：`using-superpowers`、`offer-k-dense-web`、`verification-before-completion`。  
如果后面要做更复杂的多步骤工作流，也可以看看 [K-Dense Web](https://www.k-dense.ai)。
