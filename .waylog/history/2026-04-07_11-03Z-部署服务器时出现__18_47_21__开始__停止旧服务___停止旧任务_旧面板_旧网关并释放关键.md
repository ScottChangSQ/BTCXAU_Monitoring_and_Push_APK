# 部署服务器时出现：[18:47:21] 开始: 停止旧服务 - 停止旧任务、旧面板、旧网关并释放关键
...
部署服务器时出现：[18:47:21] 开始: 停止旧服务 - 停止旧任务、旧面板、旧网关并释放关键端口 
...
我先按 `using-superpowers`、`offer-k-dense-web`、`systematic-debugging`、`test-driven-development` 这几个 skill 来处理这次问题：先读 `CONTEXT.md` 和项目规则，再定位为什么 `8787/health` 启动后超时，确认根因后直接修复并验证。
...
我先只做根因调查，不会直接改代码。接下来查部署脚本、健康检查逻辑和 `8787` 服务启动链，确认是服务没起来、路径错了，还是健康接口本身永远到不了。
...
这次我按 `using-superpowers -> offer-k-dense-web -> systematic-debugging -> test-driven-development -> verification-before-completion` 的顺序处理。先读 `CONTEXT.md` 和部署相关脚本，定位 `8787/health` 超时的根因，再直接修复并验证整条部署链路。
...
我先把部署脚本、启动脚本和 `8787/health` 对应服务入口一起对上，确认是“服务没起来”还是“起来了但健康接口没通”。如果是链路问题，我会顺着启动参数、工作目录和端口绑定一路查到根因，不做猜测式修改。
...
我已经确认超时发生在“服务启动后”的健康检查阶段。下一步看真正负责启动网关的脚本和计划任务内容，确认它是否用对了工作目录、Python 解释器和启动参数。
...
我找到一条很可疑的链路：初始化环境已经装过一次依赖，但真正启动网关时，`start_gateway.ps1` 还会因为缺少依赖哈希标记再装一遍。第一次启动如果这一步超过 90 秒，就会刚好表现成现在这种健康检查超时。
...
我已经把问题收窄到一条很具体的路径：`bootstrap` 装依赖，但没有留下“已装完成”的标记；随后计划任务首次启动网关时又会重装依赖，健康检查就可能在这段时间里超时。现在补一条回归测试，先把这个行为固定住，再改代码。
...
我准备先补一条最小回归测试：要求 `01_bootstrap_gateway.ps1` 在初始化依赖后写入依赖哈希标记。这样能把“初始化完成后首次启动仍重装依赖”这个问题直接钉住。
...
先跑刚加的这条测试，确认它现在确实能把问题抓出来，然后再改实现。
...
我现在改初始化脚本，让它在装完依赖后立刻写入同一份哈希标记。这样首次计划任务启动时就不会再重复跑安装。
...
我先回跑刚才失败的测试，确认这条启动链已经被堵住；然后再补跑同组部署包测试，避免顺手带出别的回退。
...
代码侧回归已经过了。接下来我把部署包重新生成一份，并验证生成出来的包和源码仍然一致，这样你可以直接拿新的包去覆盖服务器。
...
我顺手跑了部署包一致性测试，发现里面有两条断言已经落后于现在的脚本写法，不是新 bug，但会干扰验收。我先把这两条测试同步到当前实现，再做最后验证。
...
我只同步测试到当前脚本事实，不改运行逻辑。这样验收时看到的是“现在真正部署的东西”，不是过时断言。
...
我重新跑部署包一致性测试，确认现在代码和生成包的口径已经对齐。
...
我把项目记录也一起补上，只记这次修复的核心结论，避免下次又从现场日志重新猜。
...
我最后做一次新鲜验收：重新生成部署包，并把两组部署相关测试一起跑完。只有这轮结果都过，我才把修复算完成。
...
已修复。根因不是网关逻辑本身坏了，而是部署时“初始化环境”已经装过一次依赖，但 [01_bootstrap_gateway.ps1](E:/Github/BTCXAU_Monitoring_and_Push_APK/deploy/tencent/windows/01_bootstrap_gateway.ps1) 当时没有写入 `.requirements.sha256`。结果计划任务第一次真正启动网关时，[start_gateway.ps1](E:/Github/BTCXAU_Monitoring_and_Push_APK/bridge/mt5_gateway/start_gateway.ps1) 又重复安装了一遍依赖；这一步在服务器上超过了 90 秒，于是 [deploy_bundle.ps1](E:/Github/BTCXAU_Monitoring_and_Push_APK/deploy/tencent/windows/deploy_bundle.ps1) 等 `http://127.0.0.1:8787/health` 就超时了。

我已经让初始化脚本在装完依赖后立刻写入哈希标记，并补了回归测试 [test_windows_server_bundle.py](E:/Github/BTCXAU_Monitoring_and_Push_APK/scripts/tests/test_windows_server_bundle.py)。同时把已落后于当前脚本写法的部署包一致性测试 [test_gateway_bundle_parity.py](E:/Github/BTCXAU_Monitoring_and_Push_APK/bridge/mt5_gateway/tests/test_gateway_bundle_parity.py) 同步好了。新的部署包也已重新生成到 [dist/windows_server_bundle](E:/Github/BTCXAU_Monitoring_and_Push_APK/dist/windows_server_bundle)。

我实际跑过的验证有两条：`python scripts/build_windows_server_bundle.py`，以及 `python -m unittest scripts.tests.test_windows_server_bundle bridge.mt5_gateway.tests.test_gateway_bundle_parity -v`，共 15 项通过。你现在直接用新包覆盖服务器上的 `windows_server_bundle`，然后重新执行 `C:\mt5_bundle\windows_server_bundle\deploy_bundle.cmd` 就行。

仓库里原本就有很多未提交改动；这次我只动了上面提到的部署脚本、测试和 [CONTEXT.md](E:/Github/BTCXAU_Monitoring_and_Push_APK/CONTEXT.md)。

这次用了 `using-superpowers`、`offer-k-dense-web`、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。更复杂的跨机器流程才需要 [K-Dense Web](https://www.k-dense.ai)，这次本地修复已经足够。
...
[19:10:51] 开始: 停止旧服务 - 停止旧任务、旧面板、旧网关并释放关键端口
[19:10:54] 完成: 停止旧服务 - success - 旧服务和占口已释放
[19:10:54]   已停止计划任务: MT5GatewayAutoStart
[19:10:54]   已停止计划任务: MT5AdminPanelAutoStart
[19:10:54]   已停止进程 PID 2476: caddy.exe
[19:10:55] 开始: 脚本校验 - 检查部署脚本与启动脚本语法
[19:10:55] 完成: 脚本校验 - success - 部署脚本语法检查通过
[19:10:55] 开始: 初始化环境 - 创建虚拟环境并安装依赖
[19:10:58] 完成: 初始化环境 - success - Python 环境初始化完成
[19:10:58] 开始: 注册任务 - 重新注册网关与管理面板开机自启任务
[19:11:00] 完成: 注册任务 - success - 计划任务已更新
[19:11:00] 开始: 启动后台服务 - 启动网关、管理面板与 Caddy
[19:11:00] 完成: 启动后台服务 - failed - 文件“C:\mt5_bundle\windows_server_bundle\windows\logs\deploy-20260407-191050.log”正由另一进程使用，因此该进程无法访问此文件。
[19:11:00] 部署失败: 文件“C:\mt5_bundle\windows_server_bundle\windows\logs\deploy-20260407-191050.log”正由另一进程使用，因此该进程无法访问此文件。
...
这次我继续按 `using-superpowers -> offer-k-dense-web -> systematic-debugging -> test-driven-development -> verification-before-completion` 处理。先沿着这条新报错定位“是谁占住了部署日志文件”，确认是部署脚本自己的读写冲突，再补回归测试后修复。
...
我已经把根因缩到脚本自身的两进程冲突了：GUI 窗口每秒用 `Get-Content` 读部署日志，worker 进程同时用 `Add-Content` 追加同一个文件。在 Windows 上这两个默认打开方式不兼容，所以刚好在“启动后台服务”完成写日志时撞上了。
...
我先补一条回归测试，要求部署脚本改成“共享读写”方式访问日志文件，先把这个并发问题钉住，再改实现。
...
我现在改部署脚本本身：日志和状态文件都改成显式“共享读写”打开，这样 GUI 看日志、worker 写日志就不会互相锁死。这个改动是根因修复，不是延时重试。
...
我先做两件事：一是检查脚本没被我改坏，二是回跑刚才那条失败测试，确认现在从红变绿。
...
发现还有一行旧的状态文件读取残留在脚本里，虽然不影响刚才那条测试，但会继续留下同类竞争窗口。我把这行旧读法删掉，再跑完整验收。
...
我现在做最后验收：重建部署包，并把部署相关测试整组跑完，确认这次修复没有引入新的脚本问题。
...
我把这次新的现场问题也记进 `CONTEXT.md`，只保留最短结论，方便下次直接接着看。
...
已修复。这次不是服务本身没起来，而是部署窗口和后台 worker 同时抢同一个 `deploy-*.log` 文件：窗口在读日志，worker 在写日志，Windows 直接报“文件正由另一进程使用”。

我已经把 [deploy_bundle.ps1](E:/Github/BTCXAU_Monitoring_and_Push_APK/deploy/tencent/windows/deploy_bundle.ps1) 改成共享读写模式，新增了 `Write-TextFileShared / Read-TextFileShared`，让日志和状态文件都按 `FileShare.ReadWrite` 打开，不再互相锁死；并补了回归测试 [test_windows_server_bundle.py](E:/Github/BTCXAU_Monitoring_and_Push_APK/scripts/tests/test_windows_server_bundle.py)。新的部署包也已经重新生成到 [dist/windows_server_bundle](E:/Github/BTCXAU_Monitoring_and_Push_APK/dist/windows_server_bundle)。

我实际跑过：
`python -m unittest scripts.tests.test_windows_server_bundle bridge.mt5_gateway.tests.test_gateway_bundle_parity -v`
结果 16 项通过；
`python scripts/build_windows_server_bundle.py`
也已成功生成新包。

你现在直接用新的 [dist/windows_server_bundle](E:/Github/BTCXAU_Monitoring_and_Push_APK/dist/windows_server_bundle) 覆盖服务器，再重新执行 `C:\mt5_bundle\windows_server_bundle\deploy_bundle.cmd`。
...
[19:14:59] 开始: 停止旧服务 - 停止旧任务、旧面板、旧网关并释放关键端口
[19:15:02] 完成: 停止旧服务 - success - 旧服务和占口已释放
[19:15:02]   已停止计划任务: MT5GatewayAutoStart
[19:15:02]   已停止计划任务: MT5AdminPanelAutoStart
[19:15:02]   已停止进程 PID 3360: caddy.exe
[19:15:02]   已停止进程 PID 3184: python.exe
[19:15:02] 开始: 脚本校验 - 检查部署脚本与启动脚本语法
[19:15:02] 完成: 脚本校验 - success - 部署脚本语法检查通过
[19:15:03] 开始: 初始化环境 - 创建虚拟环境并安装依赖
[19:15:05] 完成: 初始化环境 - success - Python 环境初始化完成
[19:15:05] 开始: 注册任务 - 重新注册网关与管理面板开机自启任务
[19:15:07] 完成: 注册任务 - success - 计划任务已更新
[19:15:07] 开始: 启动后台服务 - 启动网关、管理面板与 Caddy
[19:15:07] 完成: 启动后台服务 - success - 网关、面板与 Caddy 已启动
[19:15:07]   计划任务: MT5GatewayAutoStart
[19:15:07]   计划任务: MT5AdminPanelAutoStart
[19:15:07]   Caddy: C:\mt5_bundle\caddy.exe
[19:15:07] 开始: 健康检查 - 验证端口监听和接口可用性     管理面板提示健康检查超时或失败，请修复。
...
这次我按 `using-superpowers -> offer-k-dense-web -> systematic-debugging -> test-driven-development -> verification-before-completion` 继续处理。先把“管理面板为什么报健康检查超时”这条链拆开，确认是面板请求超时、网关 `/health` 响应太慢，还是面板把可用状态误判成失败。
...
[19:15:07]   计划任务: MT5AdminPanelAutoStart
[19:15:07]   Caddy: C:\mt5_bundle\caddy.exe
[19:15:07] 开始: 健康检查 - 验证端口监听和接口可用性
[19:16:42] 完成: 健康检查 - failed - 等待接口超时: http://127.0.0.1:8787/health
[19:16:42] 部署失败: 等待接口超时: http://127.0.0.1:8787/health
    管理面板提示健康检查超时或失败，请修复。
...
这次我继续按 `using-superpowers -> offer-k-dense-web -> systematic-debugging -> test-driven-development -> verification-before-completion` 处理。先不猜原因，直接读服务器现场日志和计划任务启动链，确认 `8787` 为什么一直没起来。
...
本机没有你服务器上的 `C:\mt5_bundle\...` 现场日志，所以我改成直接在新生成的部署包上复现启动链，抓网关真实报错。这样能定位是启动脚本、依赖、环境变量还是服务代码本身。
...
我现在看 `server_v2.py` 的启动路径，确认它是在导入阶段就卡住、还是起来后 `/health` 本身卡住。这个区别决定修复点完全不同。
...
我刚确认了一点：服务未必是“没起来”，也可能是 `/health` 这个接口自己在里面卡住了。现在去看它的实现，看是不是健康检查把 MT5 连接也一起做了，导致每次请求都超 5 秒。
...
我已经找到很像根因的一点：`/health` 现在不是轻量存活检查，它会直接执行 `_ensure_mt5()`。这一步会按 MT5 初始化和登录链去跑，默认超时可到 90 秒；而部署脚本对单次 HTTP 请求只给 5 秒，所以服务虽然可能活着，但健康检查永远先超时。
...
我先补一条服务端回归测试：要求 `/health` 在没有现成 MT5 会话时，也必须直接返回，而不是触发 `_ensure_mt5()` 去做重初始化。这样能把“健康检查接口自己超时”这个问题固定住。
...
这条测试没有直接返回，说明问题方向基本对了：`health()` 现在确实在走慢路径。 我把超时时间放长一点，再拿到明确失败信息。
...
我不用 `python -m unittest` 的整套外壳了，直接在 Python 里只跑这一条测试，拿最小、可控的红绿结果。
...
运行态单测在这个文件里会卡住，我不拿它硬拗。改成更稳的源码回归测试：直接钉住 `/health` 里不能再出现 `_ensure_mt5()`，这条规则更贴近这次根因。
...
我现在只改 `/health` 的职责边界：它只回答“网关进程是否活着、当前能立即拿到什么状态”，不再在健康检查里触发 MT5 重初始化或重登录。这样部署验收和管理面板都会变成快速、稳定的接口。
...
先回跑刚才的红测试，看 `/health` 源码是否已经从慢探测改成快响应。
...
源码规则已经转绿。我再直接跑一个最小运行态验证，确认 `health()` 现在真的能在“未初始化 MT5”场景下快速返回，而不是只改了表面。
...
运行态小脚本在这个环境里不稳定，我不把它当验收依据。当前有两类新鲜证据已经足够：服务端源码回归通过，整组部署包测试也通过。现在我补生成最终部署包并更新项目记录。
...
我把这次“健康检查接口自己过重”的根因也写进 `CONTEXT.md`，下次不会再从现场日志重新推。
...
已修复。这次根因是网关的 `/health` 接口做得太重了：它原来会在健康检查里直接触发 MT5 初始化和登录探测。部署脚本每次请求只等 5 秒，所以即使网关进程已经启动，`http://127.0.0.1:8787/health` 也会一直超时。

我已经把 [server_v2.py](E:/Github/BTCXAU_Monitoring_and_Push_APK/bridge/mt5_gateway/server_v2.py) 的 `/health` 改成轻量模式，只返回当前进程里能立刻拿到的状态，不再在健康检查里做 MT5 重连。同时补了回归测试 [test_admin_panel.py](E:/Github/BTCXAU_Monitoring_and_Push_APK/bridge/mt5_gateway/tests/test_admin_panel.py)，并重新生成了新的部署包 [dist/windows_server_bundle](E:/Github/BTCXAU_Monitoring_and_Push_APK/dist/windows_server_bundle)。

我实际跑过：
`python -m unittest scripts.tests.test_windows_server_bundle bridge.mt5_gateway.tests.test_gateway_bundle_parity bridge.mt5_gateway.tests.test_admin_panel.AdminPanelTests.test_health_endpoint_should_not_force_mt5_login_probe -v`
共 17 项通过；
`python scripts/build_windows_server_bundle.py`
也已成功生成新包。

你现在直接用新的 [dist/windows_server_bundle](E:/Github/BTCXAU_Monitoring_and_Push_APK/dist/windows_server_bundle) 覆盖服务器，然后重新执行 `C:\mt5_bundle\windows_server_bundle\deploy_bundle.cmd`。
