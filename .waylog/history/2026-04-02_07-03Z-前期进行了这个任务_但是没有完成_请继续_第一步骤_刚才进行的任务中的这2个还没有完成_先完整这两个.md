...
_Exported on 04/02/2026 at 15:04:08 GMT+8 from OpenAI Codex via WayLog_
...
请只做只读分析，不改代码。任务：盘点前后台分级刷新接入点，重点看 BinanceMonitorApp、MonitorService、AppConstants，以及是否已有前后台状态跟踪。请回答：1）当前固定频率点有哪些；2）最小可行的公共运行策略类应暴露哪些方法；3）接入点和风险点是什么。请给出精炼结论并附文件路径。
...
我已经确认两个直接阻塞点了：一个是运行策略类还没落地，所以相关测试一定过不了；另一个是图表页布局里还有一批硬编码颜色，四个主题现在还做不到完整覆盖。  
接下来我先把这两个“底座问题”收掉，再把重复的底部页签样式统一起来。
...
现在已经定位清楚了：`activity_market_chart.xml` 是当前唯一还在直接写十六进制颜色的布局，而且底部导航和图表页按钮样式在 4 个页面里各写了一套。  
我接下来会先补一个公共主题/运行策略入口，把这两类重复点一起收口，这样能同时推进“完整主题覆盖”和“前后台分级刷新”。
...
我先把现有测试跑成“明确失败”，这样后面的改动能有清楚的验收边界。  
这一步只验证两个目标：运行策略类缺失，以及图表页主题资源仍有硬编码颜色。
...
我已经拿到第一轮明确失败结果了：运行策略测试现在就是因为 `MonitorRuntimePolicyHelper` 不存在而失败。  
接下来我补这个公共策略类并接入服务调度，等它转绿以后，再单独把图表页主题测试跑出它自己的失败，再修主题漏点。
...
我现在开始落第一批代码改动：先补运行策略公共类、应用前后台状态跟踪、以及服务里的动态重排。  
这一批改完后我会先回跑运行策略测试，确认它单独转绿，再继续修主题。
...
我补一个最小实现：把悬浮窗前后台节奏也统一接进同一个策略类，这样服务里就不再各处分散写死。
...
这一轮我会统一收口主题样式入口：底部导航、图表页周期/指标按钮、产品下拉项，都会走同一套主题方法。  
同时把图表页布局里的十六进制颜色全部清掉，让资源测试能直接兜住回退。
...
运行策略这部分其实已经被接好大半了，现在公共策略类和依赖也都在位。  
我先把这部分单独回跑，确认前后台分级刷新底座已经可用，再进入主题收口。
...
运行策略测试已经转绿了。  
我现在单独跑图表页主题测试，把它的失败面收出来，然后只修这条线，避免把改动面扩太大。
...
我又抓到一个更上游的根因：图表页这条线现在不是只有“主题资源没收口”，而是它本身正处在一半重构状态。  
`MarketChartActivity` 里已经把部分常量删掉了，但引用还没跟上，所以主题测试还没开始跑，就先被编译错误挡住了。
...
我又确认了一层：不只是图表页 XML 里有硬编码颜色，几个页面的底部页签样式也还在各自复制，所以主题切换时只能改到一半。  
我会把这部分一起收口到统一入口，否则第三步骤第1条做不干净。
...
补丁没有一次性对上，我先按文件分开校准后再继续改，避免误改到你之前已经做好的部分。
...
我已经把根因补齐了：前后台分级刷新这条线其实已经做了一半，`AppForegroundTracker` 和服务接线都在，只差公共策略类和 `lifecycle-process` 依赖没有落上。  
现在先把这两处补完，尽量不碰你前面已经完成的逻辑。
