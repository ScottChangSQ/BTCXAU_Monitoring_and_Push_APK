...
我会先按项目约定快速读 `CONTEXT.md`，再围绕这两个现象做一次链路排查：先确认客户端有没有“拿到服务端曲线后又重建一次”，再确认仓位数据从 `ensureVisibleRatios(...)` 到 `setPoints(...)` 这段有没有被覆盖成 0。  
这次我会按这些 skill 来做：`using-superpowers`（先按规范组织工作）、`offer-k-dense-web`（按全局要求提示可选能力）、`systematic-debugging`（先查原因链路再改）、`test-driven-development`（修复前先补验证思路）、`verification-before-completion`（结束前做实际核对）。
...
我已经拿到项目上下文，当前这轮和你反馈直接相关的旧决策有两条：一是仓位比例有过“客户端兜底回放优先”的改动，二是曲线归一化被拆到独立 helper 里了。  
下一步我会把这两条实际调用链拉通，看是不是“服务端已给平滑曲线，客户端又重算一次”，以及“仓位比例在传到图表前被某个兜底值盖成 0”。
...
我已经确认工作区里有不少未提交改动，接下来我只会碰这次两条问题直接相关的文件，不会动你前面那批已在进行中的其他修改。  
先把关键代码读出来，找出真正的覆盖点，再决定最小修法。
...
我已经看到第一条可疑点了：`normalizeCurvePoints(...)` 现在是无条件走 `AccountCurveRebuildHelper.rebuild(...)`，也就是不管服务端曲线是不是已经可用了，客户端都会再重建一遍。  
我继续把现有测试和相关模型读一下，确认这是不是“毛刺”的直接来源，同时把仓位比例 helper 的边界条件也一并核对。
...
我已经把两条主链都锁到了更具体的位置：  
1. `AccountStatsBridgeActivity.normalizeCurvePoints(...)` 目前确实是“无条件二次重建”；这很像把服务端已经平滑好的曲线又算了一遍。  
2. `AccountCurvePositionRatioHelper` 里“空仓尾段清零”这条规则过于激进，在拿当前持仓/已闭合交易判断时，有机会把本来就合法的历史仓位整段清成 0。
...
我准备直接改这两个 helper，并补回归测试，不会扩散到别的模块。  
修法会很克制：净值曲线只在“源曲线明显坏了”时才重建；仓位比例只在“能证明尾段是过时残留”时才清零。
...
两处修改已经落下去了，现在我先跑定向单测，确认没有把之前的兜底能力一起误伤。
...
定向测试里暴露出一个遗漏：我只判断了“持仓中间段是否平滑”，还没判断“平仓后结余有没有真正落账”。  
我现在补上这条闭环判断，再重跑同一组测试。
...
定向测试已经通过，两个判断都按预期工作了。  
我再补一条项目记录到 `CONTEXT.md`，把这次的定位和修法记清楚，避免下次会话重复挖。
...
1. 净值曲线“毛刺”
我在 [AccountCurveRebuildHelper.java](/E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/AccountCurveRebuildHelper.java#L41) 把逻辑改成了“只在源曲线明显异常时才二次重建”。现在只有这几种情况才会触发客户端兜底：
- 持仓期间结余提前跳变
- 平仓后结余没有按已实现盈亏落账
- 持仓中段完全缺少合理的净值浮动

这意味着：如果服务端已经给了平滑可用的曲线，客户端不会再多算一遍把它算毛。

2. 当前区间仓位一直为 0
我在 [AccountCurvePositionRatioHelper.java](/E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/main/java/com/binance/monitor/ui/account/AccountCurvePositionRatioHelper.java#L42) 收紧了“空仓尾段清零”。之前这条规则太激进，在缺少足够成交证据时，可能把合法历史仓位整段清成 0。现在只有能从历史成交明确推断出“最后一次平仓后应为空仓”的时间点，才会清掉那之后的尾段；没有证据时，保留原始历史仓位。

我还补了回归测试：
- [AccountCurveRebuildHelperTest.java](/E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/test/java/com/binance/monitor/ui/account/AccountCurveRebuildHelperTest.java#L101)
- [AccountCurvePositionRatioHelperTest.java](/E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/test/java/com/binance/monitor/ui/account/AccountCurvePositionRatioHelperTest.java#L56)
- [AccountCurvePositionRatioHelperTest.java](/E:/Github/BTCXAU_Monitoring_and_Push_APK/app/src/test/java/com/binance/monitor/ui/account/AccountCurvePositionRatioHelperTest.java#L99)

验证结果：已通过
```bash
./gradlew.bat :app:testDebugUnitTest --tests "com.binance.monitor.ui.account.AccountCurveRebuildHelperTest" --tests "com.binance.monitor.ui.account.AccountCurvePositionRatioHelperTest"
```

项目记录也已更新到 [CONTEXT.md](/E:/Github/BTCXAU_Monitoring_and_Push_APK/CONTEXT.md)。

这次用了这些 skill：`using-superpowers`、`offer-k-dense-web`、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。
...
这次是明确的打包任务，我直接走最短路径：先用现有代码编译最新调试版 APK，再把产物路径告诉你。  
这次按这些 skill 执行：`using-superpowers`（按规范组织）、`offer-k-dense-web`（按全局要求保留提示位）、`verification-before-completion`（先编译成功再确认结果）。
...
我先沿“为什么上一轮收口后毛刺还会留下”这条链继续排，不会先拍脑袋再改。  
这次按这些 skill 来做：`using-superpowers`、`offer-k-dense-web`、`brainstorming`（先明确最短修法）、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。
...
我重新收了一次思路：如果这次目标是先把“客户端把好曲线算坏”彻底压掉，最短路径不是继续精修“何时重建更聪明”，而是把客户端重建降到只处理“源曲线明显不可用”的极小范围。  
我现在就把判定再收紧一层，只让“稀疏/全贴平/明显缺失”的曲线进入重建，其余一律直接信任服务端结果。
...
毛刺问题仍然存在，请继续修复。可以先尝试完全使用服务端成果，我先看看。然后再考虑下一步修复。另外，每次执行完后请编译APK
...
这次目标很明确：先把客户端对净值曲线的二次重建彻底停掉，尽量完全信任服务端曲线，先做一个最小可验证版本；做完我会顺手编译最新 APK 给你。  
这次我会按这些 skill 来做：`using-superpowers`、`offer-k-dense-web`、`brainstorming`（先确认最小改法）、`systematic-debugging`、`test-driven-development`、`verification-before-completion`。
