...
class WindowsServerBundleTests(unittest.TestCase):
    ...
    def test_deploy_script_should_upgrade_gui_into_runtime_status_panel(self) -> None:
        ...
        self.assertIn("$localBox.WordWrap = $false", deploy_script)
        self.assertIn("$marketBox.WordWrap = $false", deploy_script)
        self.assertIn("$accountBox.WordWrap = $false", deploy_script)
        self.assertIn("$appBox.WordWrap = $false", deploy_script)
